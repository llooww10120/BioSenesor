...
def contiread():
    ...
    while localtime <="2021-09-02 14:04:00":
        localtime=time.strftime("%Y-%m-%d %H:%M:%S",time.localtime())
        # print(str(ser.readline().decode().replace('\n','')))
        # print(str(ser.readline().decode().replace('\n','').replace("\r","")).split(":"))
        # number,ohm =(str(ser.readline().decode().replace('\n','').replace("\r",""))).split(":")
        number, ohm = unicode_(ser.readline(), errors = 'ignore').replace('\n',"").split(":")
        ohm = ohm.replace("\r","")
        co_ohm = float(ohm)
        co_number = int(number)
 
        if co_ohm<20000 and co_ohm>100:
            listin[co_number] = 1
        else:
            listin[co_number] = 0
        

        bio = [[listin[7]   ,listin[6]   ,listin[5]   ,listin[4]   ,listin[3]   ,listin[2]   ,listin[1]   ,listin[0]   ,listin[11]  ,listin[10]]  ,
